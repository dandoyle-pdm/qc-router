...
## Iteration Protocol (ARC-AGI Pattern)

When working on iteration tickets (sequence > 001), follow the structured learning protocol to avoid repeating failed approaches and target specific gaps.

**Full details**: See [ITERATION_PROTOCOL.md](./ITERATION_PROTOCOL.md)

**Key points**:
- Check ticket frontmatter for `iteration.index > 1`
- Read Attempt History and Structured Feedback sections
- Prioritize lowest-scoring requirements first
- Follow guidance precisely - it specifies exact fixes needed
- Track accuracy trajectory to measure progress
...
|--------------|-------------------|-----------------|--------|
| Production code files | N/A | Context-dependent | Project standards |
| Test files | N/A | Context-dependent | Project standards |
...
- [ ] **Tests exist**: New logic paths have corresponding tests
- [ ] **Tests pass**: `npm test` or equivalent succeeds
- [ ] **No hardcoded secrets**: Grep for API keys, passwords, tokens
- [ ] **Error handling**: All error paths have appropriate handling
...
2. **DECOMPOSE** - Break oversized artifacts into smaller units
...
```
Validation Results:
- Tests: [PASS/FAIL] - [X] tests, [Y] passing
- Lint: [PASS/FAIL]
